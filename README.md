...
├── configs/ # [配置层] 存放系统参数
│ ├── **init**.py # ⚠️ 必须有 (空文件即可)
│ └── system_config.py # 系统运行时的配置文件 (存 RTSP 地址、报警阈值等)
│
├── core/ # [算法层] 核心逻辑
│ ├── **init**.py # ⚠️ 必须有
│ ├── detector.py # 负责调用 YOLO、绘图、计数逻辑
│ ├── speed_estimator.py # 负责速度计算 (透视变换)
│ ├── attention.py # 修改 YOLO 架构，添加手写注意力层
...
│
├── data/ # [数据层] 资源文件
│ ├── visdrone/ # 数据集目录 (Roboflow 下载的或解压的)
│ │ ├── train/
│ │ ├── valid/
│ │ ├── test/
│ │ └── data.yaml # ⚠️ 训练用的数据集配置文件
│ │
│ └── test_video1.mp4 # 测试用的视频 (GitHub 上传时会被忽略)
│
